...
// REGLAS 2 y 3: Procesamiento del carrito (Cálculo de IGV por categoría y subtotales)
$total_subtotal_neto = 0;
$total_igv_acumulado = 0;
$detalle_procesado   = [];

foreach ($productos as $prod) {
    $subtotal = $prod['precio'] * $prod['cantidad'];
    
    // Switch para determinar tasa de IGV por categoría
    switch (strtolower($prod['categoria'])) {
        case 'abarrotes':
        case 'bebidas':
        case 'lácteos':
        case 'limpieza':
        ...
        case 'frutas y verduras':
            ...
            break;
        default:
            $tasa_igv = 0.18; // Por defecto general
            break;
    }
    ...
    // Acumuladores de los totales globales
    $total_subtotal_neto += $subtotal;
    $total_igv_acumulado += $igv_producto;
    
    // Almacenamos el cálculo estructurado para el render HTML
    $detalle_procesado[] = [
        "nombre"   => $prod['nombre'],
        "precio"   => $prod['precio'],
...
// El monto base sobre el cual se calculan los descuentos (Subtotal + IGV)
$monto_base_calculo = $total_subtotal_neto + $total_igv_acumulado;

// REGLA 4: Descuento por monto total (if/elseif)
$porcentaje_desc_monto = 0;
if ($monto_base_calculo < 30) {
    $porcentaje_desc_monto = 0.00;
} elseif ($monto_base_calculo >= 30 && $monto_base_calculo < 100) {
    $porcentaje_desc_monto = 0.05;
} elseif ($monto_base_calculo >= 100 && $monto_base_calculo < 200) {
    $porcentaje_desc_monto = 0.10;
} else {
    $porcentaje_desc_monto = 0.15; // S/ 200 a más
}
$descuento_monto = $monto_base_calculo * $porcentaje_desc_monto;

// REGLA 5: Descuento adicional por tipo de cliente (if/elseif)
$porcentaje_desc_cliente = 0;
if (strtolower($cliente_tipo) === 'frecuente') {
    $porcentaje_desc_cliente = 0.02;
} elseif (strtolower($cliente_tipo) === 'vip') {
    $porcentaje_desc_cliente = 0.05;
} else {
    $porcentaje_desc_cliente = 0.00; // Regular
}
$descuento_cliente = $monto_base_calculo * $porcentaje_desc_cliente;

// Totales de descuentos
$total_descuentos = $descuento_monto + $descuento_cliente;
$total_a_pagar   = $monto_base_calculo - $total_descuentos;

// REGLA 6: Validación de método de pago (switch)
$instruccion_pago = "";
switch (strtolower($metodo_pago)) {
    case 'efectivo':
        $instruccion_pago = "Pago en efectivo - exacto preferido";
        if ($total_a_pagar > 500) {
            $instruccion_pago .= " (ADVERTENCIA: Se sugiere otro método para montos altos)";
        }
        break;
    case 'yape':
    case 'plin':
    ...
    case 'tarjeta':
        ...
        break;
    default:
        $instruccion_pago = "Método de pago no registrado";
        break;
}
